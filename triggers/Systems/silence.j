function Trig_silence_Actions takes nothing returns nothing
  ...
  if TestMode then 
  call SilenceUnit(Hero[i],Hero[i],3)
    endif
endfunction
//===========================================================================
function InitTrig_silence takes nothing returns nothing
    ...
    loop
        exitwhen i == 13
        call TriggerRegisterPlayerChatEvent( LvlSelf, Player(i), "-silence", true )
       set i = i + 1
    endloop
    call TriggerAddAction( LvlSelf, function Trig_silence_Actions )
    set LvlSelf = null
endfunction

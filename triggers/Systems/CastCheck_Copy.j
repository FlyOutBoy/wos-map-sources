...
function SpellExtension takes unit c, integer id returns boolean then 
...
//call MyFlush(GetHandleId(Player(i)),StringHash("ahk pidor"),0,ahk_delay )
    ...
    set i2 = 1 
    call CD_Start(c,0.03,id,BlzGetUnitAbilityCooldown(c,id,GetUnitAbilityLevel(c,id)-1)*(1-TojiG_CdReduce/100))
    endif
    if check == 1 and IsItemInInventory(c,'I011')>0 and GetMainStatInt(c) then
    set i2 = 1
    call CD_Start(c,0.03,id,OkarunEggCdReal(c,BlzGetUnitAbilityCooldown(c,id,GetUnitAbilityLevel(c,id)-1)))
    endif
    if check == 1 and BlzGetUnitAbilityCooldown(c,id,GetUnitAbilityLevel(c,id)-1)>=3 and  BlzGetUnitAbilityCooldownRemaining(c,'A0BZ')==0 and GetMainStatAgi(c) and IsItemInInventory(c,'I024')>0 then
    if i2 == 0 then 
    call CD_Start(c,0.03,id,OkarunEggCdReal(c,BlzGetUnitAbilityCooldown(c,id,GetUnitAbilityLevel(c,id)-1)))
    endif
    ...
    call CD_Start(c,0.03,id,BlzGetUnitAbilityCooldown(c,id,GetUnitAbilityLevel(c,id)-1)*0.9)
    endif
...
//===========================================================================
function InitTrig_CastCheck_Copy takes nothing returns nothing
    local integer index = 0
    set gg_trg_CastCheck_Copy = CreateTrigger()
    call TriggerAddCondition( gg_trg_CastCheck_Copy, Condition( function CastHero_Conditions ) )
endfunction

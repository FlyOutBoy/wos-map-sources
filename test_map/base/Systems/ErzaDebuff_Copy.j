library AAADest uses GearSystems
    globals
        ...
        real ErzaWape_IncreaseDmg = 9 // increase physical taken dmg by target with wape debuff by 8% ( 8 = 8%)
        ...
        real ErzaFire_DamageBase = 50 // static damage each sec
        real ErzaWape_Damage = 1.5 // x mainstat damage each one time
        real ErzaOverload_Damage = 1.5 // x mainstat damage each one time
        real ErzaElectrized_Damage = 0.5 // x mainstat damage each sec for buff duration time
        real ErzaOverload_Stun = 1.25 // increased stun when water + fire
        real ErzaLightning_Stun = 0.5 // stun time after end of buff
        real ErzaLightning_ManaBurnBase = 100 // burn mana after end of buff
    ...
    function SlowUnitErza takes unit c, unit u, integer lvl returns nothing // percent - from 10 to 80( all percentage, example 10, 20, 30, 40, 50, 60, 70, 80), time choose prefered time 2, 3, 4 sec only
        ...
        if DummyPlayer2[i] == null or GetWidgetLife(DummyPlayer2[i]) < 1 then
            ...
            call UnitAddAbility(DummyPlayer2[i], 'A03U')
        endif
        if lvl< 1 then 
        set lvl = 1
        endif
        if lvl > 0 then
            if GetUnitAbilityLevel(DummyPlayer2[i], 'A03U') == 0 then
                call UnitAddAbility(DummyPlayer2[i], 'A03U')
            endif
            call SetUnitAbilityLevel(DummyPlayer2[i], 'A03U', lvl)
            call SetUnitFacing(DummyPlayer2[i], GAngle(DummyPlayer2[i], u) * bj_RADTODEG)
            call IssueTargetOrder(DummyPlayer2[i], "slow", u)
        endif
    endfunction
    private struct ErzaDebuffs
        ...
        private static method Loop_ErzaBar takes nothing returns nothing
            ...
            loop
                exitwhen i > MUI_3
                set this = m_3[i]
                if GetUnitTypeId(Hero[k2]) == id then
                    set r = r + 0.05
        ...
        public static method ErzaBar_Start takes unit NewC returns nothing
            ...
            set frame2_pas1[k2] = BlzCreateFrameByType("BACKDROP", "SS", BlzGetFrameByName("ConsoleUIBackdrop", 0), "", 0)
                ...
                if r > 0.06 then
                    if GetUnitAbilityLevel(td, id) > 0 then
                        ...
                        if r2 > 0.9 then
                            ...
                            if (k2 == 2 or k2 == 6 ) and r < 4.4 then
        ...
        public static method ErzaPas_Start takes unit NewC, unit NewTd , integer NewId returns nothing
            ...
            if MUI_ErzaPas == 0 then
                call TimerStart( t_ErzaPas, 0.03, true, function thistype.Loop_Elemental )
            endif
        endmethod
    endstruct
    ...
    function ErzaPassive takes unit c, unit u, integer level returns nothing // min 0.5 , max 5.0 sec
        ...
        if GetUnitTypeId(u) == Rimuru_ID then 
        if level == 1 then 
        set level = 0
        endif
        endif
        if IsUnitIllusion(u) == false and IsUnitVisible(u, Player(i)) == true and LoadInteger(hs,GetHandleId(u),StringHash("naofumi shield")) == 0 and level> 0  then
            if DummyPlayer2[i] == null or GetWidgetLife(DummyPlayer2[i]) < 1 then
                set DummyPlayer2[i] = CreateUnit( Player( i ), 'h0C9', GetRectCenterX( gg_rct_Caster ), GetRectCenterY( gg_rct_Caster ), 0 )
                call UnitAddAbility(DummyPlayer2[i], ErzaElemntalAbi_ID)
            endif
            if level > 0 then
                if GetUnitAbilityLevel(DummyPlayer2[i], ErzaElemntalAbi_ID) == 0 then
                    call UnitAddAbility(DummyPlayer2[i], ErzaElemntalAbi_ID)
                endif
                call SetUnitFacing(DummyPlayer2[i], GAngle(DummyPlayer2[i], u) * bj_RADTODEG)
                if level == 1 then
                ...
                call SetUnitAbilityLevel(DummyPlayer2[i], ErzaElemntalAbi_ID, level)
                if GetUnitAbilityLevel(u, ErzaWape_ID) > 0 or GetUnitAbilityLevel(u, ErzaOverload_ID) > 0 or GetUnitAbilityLevel(u, ErzaElectrized_ID) > 0 then
                    set cancel = 1
                endif
                ...
                if cancel == 0 then
                    call IssueTargetOrder(DummyPlayer2[i], "curse", u)
                    if level == 2 and BlzGetUnitAbilityCooldownRemaining(c,'A081')==0 and IsItemInInventory(c,'I01P')>0  then 
    ...
    function decordestroy takes nothing returns nothing
        ...
        local real rand = -225
        local real add_angle = 0
        local real hpcheck = hp - decordmg
        local unit u = null
        local real scale = 0
        local group g = null
        ...
        local boolean b = true
        local real r5
        if GetUnitTypeId(decorunit) == 'h005' then
            if SR0(decor_x, decor_y, x, y) > decor_aoe and GetDestructableLife(d) > 0 then
                set b = true
        ...
        if b then
            if LoadInteger(hs, GetHandleId(decorunit), StringHash("height td")) == 1 and hp > 0 then 
            call SaveInteger(hs, GetHandleId(decorunit), StringHash("decor add"),1)
            endif
            ...
            if GetDestructableLife(d) > 0 and IntegerCd(decorunit, I2S(GetHandleId(d)), time) then
                call SetDestructableLife(d, hp - decordmg)
                if hpcheck <= 0 and (IsItemInInventory(decorunit, 'I00Q') > 0 or IsItemInInventory(decorunit, 'I01K') > 0) then
                if IsItemInInventory(decorunit, 'I01K') > 0 then
                if BlzGetUnitAbilityCooldownRemaining(decorunit, 'A07N') == 0 then
                    call SetItemCharges(UnitItemInSlot(decorunit, IsItemInInventory3(decorunit, 'I01K')), GetItemCharges(UnitItemInSlot(decorunit, IsItemInInventory3(decorunit, 'I01K'))) + 1)
                    if GetItemCharges(UnitItemInSlot(decorunit, IsItemInInventory3(decorunit, 'I01K'))) >= 12 then
                        call SetItemCharges(UnitItemInSlot(decorunit, IsItemInInventory3(decorunit, 'I01K')), 0)
                        call BlzStartUnitAbilityCooldown(decorunit, 'A07N', 5)
                    endif
                    call SetHpCurrent(decorunit, 90)
                    call SetMpCurrent(decorunit, 40)
                endif
                else
                if BlzGetUnitAbilityCooldownRemaining(decorunit, 'A02E') == 0 then
                    call SetItemCharges(UnitItemInSlot(decorunit, IsItemInInventory3(decorunit, 'I00Q')), GetItemCharges(UnitItemInSlot(decorunit, IsItemInInventory3(decorunit, 'I00Q'))) + 1)
                    if GetItemCharges(UnitItemInSlot(decorunit, IsItemInInventory3(decorunit, 'I00Q'))) >= 10 then
                        call SetItemCharges(UnitItemInSlot(decorunit, IsItemInInventory3(decorunit, 'I00Q')), 0)
                        call BlzStartUnitAbilityCooldown(decorunit, 'A02E', 5)
                    endif
                    call SetHpCurrent(decorunit, 80)
                    call SetMpCurrent(decorunit, 20)
                endif
                endif
                endif
                ...
                if id == 'B00O' and hpcheck <= 0 then // explosive barrel
                    ...
                    call GroupDmgEff(decorunit, x, y, 350, 300, 1)
                endif
                ...
                if id == 'B003' or id == 'B004' or id == 'B00Y' or id == 'B00S' then // metal hit
                    if hpcheck <= 0 then
                        ...
                        call GroupDmgEff(decorunit, x, y, 400, 200, 3)
                        endif 
                ...
                if id == 'B014' or id == 'B015' or id == 'B017' then // busic
                    if hpcheck > 0 then
                        ...
                        call ScaleDummy(UnitSpawn(Player(PLAYER_NEUTRAL_PASSIVE), Fire_ID, x, y, 1, 1, 1.75, 45, 9), 7.5, 1.75, 0.5)
                        if id == 'B014' then
                            call GroupDmgEff(decorunit, x, y, 500, 500, 5)
                        call DestroyEffect(EffectSpawn("war3mapImported\\wos_OPm (431232).mdl", x, y, 1, 1, 0.7, 1))
                        call DestroyEffect(EffectSpawn("war3mapImported\\wos_SasukeYh-41.mdl", x, y, 1, 1, 1.5, 55))
                        else
                            call GroupDmgEff(decorunit, x, y, 500, 1000, 1)
                        endif
                ...
                if id == 'B00T' then // gidrant
                    if hpcheck <= 0 then
                        call GroupPushEff(decorunit, x, y, 350, 350, 0.42, -1, true,2)
    ...
    function DecorRemove takes unit c, real x, real y, real aoe, real dmg returns nothing
     ...
     function DecorRemoveInori takes unit c, real x, real y, real aoe returns nothing
        ...
        if (aoe > 0) then
            set dummyrect = Rect(x - aoe, y - aoe, x + aoe, y + aoe)
            call EnumDestructablesInRect(dummyrect, null, function decordestroyInori)
            call RemoveRect(dummyrect)
            set dummyrect = null
        endif
    endfunction
    ...
    function DecorRemove2 takes unit c, real x, real y, real aoe, real dmg returns nothing
        ...
        set decordmg = dmg
        set decorunit = c
        call EnumDestructablesInRect(gg_rct_Arena, null, function decordestroy)
    endfunction
endlibrary

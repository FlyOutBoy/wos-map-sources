...
//---------------Q ability (Explosive Land Mine)-------------------------------
        ...
        string AinzQ_MineModel = "war3mapImported\\wos_mineorange.mdl"
...
//---------------TQ / Q3 ability (Twin Dragon Lightning - T Mode)--------------
        ...
        string AinzTQ_PrepareEffect = "war3mapImported\\wos_obr08 (166).mdx"
        string AinzTQ_Effect = "war3mapImported\\wos_dragonhead.mdl"
        string AinzTQ_PrepareAttachment = "weapon"

//---------------W ability (Summon Death Knight)-------------------------------
        integer AinzW_ID = 'A0FK'
        integer AinzW_Unit_ID = 'h02F'
        string AinzW_MoveOrder = "move"
...
//---------------E ability (Wall of Skeleton)----------------------------------
        ...
        string AinzE_WallEffectModel = "war3mapimported\\wos_LD2209 (157).mdx"
...
//---------------E2 ability (Hold of Ribs - Lv. 5 Unlock)----------------------
        ...
        string AinzE2_EffectModel = "war3mapImported\\wos_ainze2.mdl"
...
//---------------R ability (True Dark Lightning)-------------------------------
        ...
        integer AinzR_Debuff = 3       // 0 = нет, 1 = Огонь, 2 = Вода, 3 = Электро
...
//---------------TR / R2 ability (Reality Slash - T Mode)----------------------
        ...
        string AinzTR_EffectModel = "war3mapImported\\droch (876).mdl"
...
//---------------T ability (The Goal of All Life is Death)---------------------
        ...
        string AinzT_DebuffOrder = "curse"
        real AinzT_DamageIntBase = 11.0
...
//---------------F ability (Dozen Layer Buffs)---------------------------------
        ...
        string AinzF_ChannelOrder = "magicdefense"
        string AinzF_BuffOrder = "bloodlust"
...
//---------------G ability (Fallen Down)---------------------------------------
    ...
    private struct AinzQ_KS
        ...
        private static method RefundCast takes unit NewC returns nothing
            ...
            call IssueImmediateOrder(NewC, AinzStopOrder)
        ...
        private method Explode takes nothing returns nothing
            local unit u
            local real scale = AinzQ_DamageAoe / 675.00
            local integer i = 0
            local thistype mine
            local real dx
            local real dy
        ...
        private static method CreateMine takes unit NewC, real NewX, real NewY, integer NewMineKind returns nothing
            ...
            set e = AddSpecialEffect(AinzQ_MineModel, x, y)
                        ...
                        if r4 > 0.45 then
                            ...
                            loop
                                ...
                                if IsUnitInGroup(u, g2) and IsUnitPaused(u) == false then
                                    ...
                                    if td == null and GetUnitCurrentOrder(u) != OrderId(AinzW_MoveOrder) and LoadInteger(hs, GetHandleId(u), StringHash("stop move")) == 0 then
    ...
    private struct AinzE_KS
        ...
        private method CreateE2Effects takes nothing returns nothing
            ...
            loop
                ...
                set e2Effect[index] = AddSpecialEffect(AinzE2_EffectModel, effectX, effectY)
        ...
        private method CreateWallEffects takes nothing returns nothing
            ...
            loop
                ...
                call DestroyEffect(EffectSpawn(AinzE_WallEffectModel, effectX, effectY, GetRandomReal(0.00, 359.00), 0.50, 1.50, 0.00))
                set index = index + 1
            endloop
        endmethod
                        ...
                        if r == 0.03 then
                            ...
                            loop
                                ...
                                if IsUnitEnemy(u, GetOwningPlayer(c)) then
                                    call BuffUnit01(c, u, AinzT_DebuffSpell_ID, AinzT_DebuffOrder, 1)
                                    call DestroyEffect(AddSpecialEffectTarget("Abilities\\Spells\\Other\\Stampede\\StampedeMissileDeath.mdl", u, "chest"))
                                endif
                                call GroupRemoveUnit(g, u)
                            endloop
    ...
    private struct AinzTQ_KS
            ...
            loop
                ...
                if check2 == 0 then
                    ...
                    if r >= AinzTQ_CastTime then
                        ...
                        if AinzTQ_Effect != "" then
                            set e = EffectSpawnScale(AinzTQ_Effect, x, y, a * bj_RADTODEG, 1, 1, AinzTQ_Height, 0.3, 0.01, 6)
                        endif
        ...
        public static method AinzTQ_Start takes unit NewC, unit NewTd returns nothing
            ...
            loop
                ...
                if k == 0 and AinzTQ_PrepareEffect != "" then
                    set e2 = AddSpecialEffectTarget(AinzTQ_PrepareEffect, c, AinzTQ_PrepareAttachment)
                endif
    ...
    private struct AinzTR_KS
        ...
        public static method AinzTR_Start takes unit NewC, real NewX, real NewY returns nothing
            ...
            set g3 = CreateGroup()
            set u = null
            set e = AddSpecialEffect(AinzTR_EffectModel, x + 250.00 * Cos(a), y + 250.00 * Sin(a))
            call BlzSetSpecialEffectYaw(e, a)
    ...
    private struct AinzF_KS
        ...
        public static method Loop_AinzF takes nothing returns nothing
            ...
            loop
                ...
                set remove = false
                if phase == 0 then
                    if SpellBoolCaster(c) and GetUnitCurrentOrder(c) == OrderId(AinzF_ChannelOrder) then
                        set r = RoundReal(r + 0.03, 3)
                        if r == 0.30 then
                            call thistype.ClearBuffs(c)
                            call BuffUnit01(c, c, AinzF_BuffSpell_ID, AinzF_BuffOrder, 2)
                        elseif r == 1.8 then
                        ...
                        elseif r == 2.10 then
                            call thistype.ClearBuffs(c)
                            call BuffUnit01(c, c, AinzF_BuffSpell_ID, AinzF_BuffOrder, 3)
                        elseif r == 3.3 then
                            call SetUnitAnimationByIndex(c, 9)
                        elseif r == 3.99 then
                            call thistype.ClearBuffs(c)
                            call BuffUnit01(c, c, AinzF_BuffSpell_ID, AinzF_BuffOrder, 4)
                        elseif r == 5.4 then
                            call SetUnitAnimationByIndex(c, 9)
                        elseif r == 5.79 then
                            call thistype.ClearBuffs(c)
                            call BuffUnit01(c, c, AinzF_BuffSpell_ID, AinzF_BuffOrder, 5)
                        elseif r == 6.9 then
                            call SetUnitAnimationByIndex(c, 9)
                            call DestroyEffect(e)
                        ...
                        elseif r == 7.20 then                        
                            call thistype.ClearBuffs(c)
                            call BuffUnit01(c, c, AinzF_BuffSpell_ID, AinzF_BuffOrder, 6)
                        elseif r == 8.7 then
                            call SetUnitAnimationByIndex(c, 9)
                        elseif r == 9.00 then
                            call thistype.ClearBuffs(c)
                            call BuffUnit01(c, c, AinzF_BuffSpell_ID, AinzF_BuffOrder, 7)
                            call UnitAddAbility(c, AinzF_Stats_ID)
                            call MyRemoveAbility2(c, AinzF_Stats_ID, 1, AinzF_Buff6_ID)
                            call IssueImmediateOrder(c, AinzStopOrder)
                        endif
                        if r > rmax then 
        ...
        public static method AinzF_Start takes unit NewC returns nothing
            ...
            call thistype.ClearBuffs(c)
            call BuffUnit01(c, c, AinzF_BuffSpell_ID, AinzF_BuffOrder, 1)
    ...
    function AinzW2_Start takes unit c, unit td returns nothing
        if IsUnitEnemy(td, GetOwningPlayer(c)) then 
            ...
            call BuffUnit01(c, td, AinzF_BuffSpell_ID, AinzF_BuffOrder, 8)
        endif
    endfunction
